...
vari=0
while vari<1:
  layout0=[[sg.T("LUXE CINEMAS",size=(27,1),justification='c',font=("gothic",40))],
  ...
  window0=sg.Window("LUXE CINEMAS",layout0,finalize=True)
  ...
  if eventing=="THEATRE REVIEWS":             
     ...
     while variable0<1:                                                                                             
        ...
        layout1=[
                [sg.T("LUXE CINEMAS",size=(32,1),justification='c',font=("arial",40))],
                 lst,
                [sg.T("                                                                                                                                                                                       ")],           
                                                ...
                                                if bill==0 :                                                                       
                                                        ...
                                                        if confirm=="Yes":
                                                            ...
                                                            for you in range(len(silverseats)):
                                                                if len(customer)!=0:
                                                                    customer.loc[len(customer)]=customername,number,codeno,showdate,event,even,silverseats[you][0],silverseats[you][1],"silver",str(150),strfttime,df.loc[loki]['bookingscreen']
                                                                else:
                                                                    customer.loc[len(customer)]=customername,number,codeno,showdate,event,even,silverseats[you][0],silverseats[you][1],"silver",str(150),strfttime,df.loc[loki]['bookingscreen']
                                                                
                                                            for you in range(len(premiereseats)):
                                                                if len(customer)!=0:
                                                                    customer.loc[len(customer)]=customername,number,codeno,showdate,event,even,premiereseats[you][0],premiereseats[you][1],"premre",str(170),strfttime,df.loc[loki]['bookingscreen']
                                                                else:
                                                                    customer.loc[len(customer)]=customername,number,codeno,showdate,event,even,premiereseats[you][0],premiereseats[you][1],"premre",str(170),strfttime,df.loc[loki]['bookingscreen']
                                                            ...
                                                            for you in range(len(orderedsnacksname)):
                                                                    ...
                                                                    if len(customersnacks)!=0:  
                                                                             customersnacks.loc[len(customersnacks)]=customername,number,codeno,showdate,even,orderedsnacksname[you],kasx.loc[max(kasx.index)]["type"],str(1),orderedsnacksprice[you]
                                                                    else:
                                                                        customersnacks.loc[len(customersnacks)]=customername,number,codeno,showdate,even,orderedsnacksname[you],kasx.loc[max(kasx.index)]["type"],str(1),orderedsnacksprice[you]
                                                            ...
                                                            customersnacks.to_csv("customer snacks.csv",index=False)
      ...
      while ww<1:
            ...
            if ting=="submit":
                  ...
                  if len(number)!=10  :
                       ...
                       while pain<1:
                            ktm=0
                            for guc in range(len(customer)):
                                print(customer)
                                if messi==0:
                                   
                                   if str(customer.iloc[guc]["name"])==str(customername) and  str(customer.iloc[guc]["phnno"])==str(number) and  str(customer.iloc[guc]["codenos"])==str(codeno) and ktm==0:
                                      looty=1
                                      ktm=1
                                      ...
                                      customersnacksdetails=customersnacksdetails.drop(["name","phnno","codenos","date","showtime"],axis=1)
                                      ...
                                      if eventing!='MY DETAILS':
                                            ...
                                            now=dt.datetime.now()
                                            for t in range(len(customerdetails)):
                                                print("")
                                            timeobj=customerdetails.iloc[t]['date']+" "+customerdetails.iloc[t]['programtime']
                                                ...
                                                if dumal==sg.WIN_CLOSED or dumal=="Back":
                                                    ...
                                                    for maxi in range(len(cusseatalpno)):
                                                         if vals[cusseatalpno[maxi]]==True:
                                                              ...
                                                              for kgd in range(len(df)):
                                                                     if str(df.iloc[kgd]["movie"])==customerdetails.iloc[0]["movie"] and df.iloc[kgd]["showtime"]==customerdetails.iloc[0]["showtime"] and df.iloc[kgd]["date"]==customerdetails.iloc[0]["date"] :
                                                                           df.loc[kgd,"seatsfilled"]=df.loc[kgd,"seatsfilled"] - 1
                                                                           if cusseatalpno[maxi][0] in ("K","J"):
                                                                               df.loc[kgd,"premiere"]=int(df.loc[kgd,"premiere"]) - 1
                                                                           else:
                                                                               df.loc[kgd,"silver"]=df.loc[kgd,"silver"] - 1
                                                              df.to_csv('movie timing pro.csv')
                                                           
                                                              cashreturn=cashreturn+int(customerdetails.iloc[maxi]["cost"])
                                                              customerdetails=customerdetails.drop(maxi)
                                                              customer=customer.drop(maxi)
